# agent-session-history-session-search Self-Test
...
- "Read my previous Claude conversation" (no agent-session-history involvement)
...
```bash
# 1. Validate agent-session-history installation
./scripts/validate.sh

# 2. Verify skill structure
ls -la .claude/skills/agent-session-history-session-search/
ls -la .claude/skills/agent-session-history-session-search/references/
ls -la .claude/skills/agent-session-history-session-search/scripts/

# 3. Run writing-skills validator
python .claude/skills/writing-skills/scripts/validate-skill.py .claude/skills/agent-session-history-session-search/
```

### Manual Validation

```bash
# Verify agent-session-history is working
agent-session-history status --json | jq '.index.fresh'
# Should return: true

# Test the core workflow
agent-session-history search "*" --workspace /data/projects/YOUR_PROJECT --aggregate agent,date --limit 1 --json
```
...
| Stale results | Index not refreshed | Run `agent-session-history index --json` |
