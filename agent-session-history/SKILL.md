...
# agent-session-history Session Search
...
1. Bootstrap: Check health, refresh index, get project overview
   agent-session-history status --json && agent-session-history index --json
   agent-session-history search "*" --workspace /data/projects/PROJECT --aggregate agent,date --limit 1 --json

2. Find prompts: Search for keywords, filter to user prompts (lines 1-3)
   agent-session-history search "KEYWORD" --workspace /data/projects/PROJECT --json --fields minimal --limit 50 \
     | jq '[.hits[] | select(.line_number <= 3)]'

3. Follow hits: View the actual content
   agent-session-history view /path/from/source_path.jsonl -n LINE -C 20

4. Expand context: See the full conversation flow
   agent-session-history expand /path/from/source_path.jsonl --line LINE --context 3

5. Discover related: Find the whole work cluster
   agent-session-history context /path/from/source_path.jsonl --json
```
...
agent-session-history search "*" --workspace /path --aggregate agent,date --limit 1 --json

# Find keyword, minimal output
agent-session-history search "KEYWORD" --workspace /path --json --fields minimal --limit 50

# Follow a hit
agent-session-history view /path.jsonl -n LINE -C 20        # Line-oriented
agent-session-history expand /path.jsonl --line LINE --context 3  # Message-oriented

# Find related sessions
agent-session-history context /path.jsonl --json

# Export for parsing
agent-session-history export /path.jsonl --format json --include-tools -o /tmp/out.json
```
...
| Full conversation | `agent-session-history expand --context 3` | Message boundaries preserved |
| Raw JSON parsing | `agent-session-history export --include-tools -o file.json` | Never pipe exports |
| Content not found | `rg "string" /path.jsonl` | agent-session-history skips tool outputs |
...
| `./scripts/validate.sh` | Validate agent-session-history is working |
...
```bash
# Quick health check
agent-session-history status --json | jq '.index.fresh'

# Should return: true
```

If `false`, run: `agent-session-history index --json`
