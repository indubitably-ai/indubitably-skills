...
name: command-guard
description: >-
  Handle blocked destructive commands. Use when command-guard blocks rm -rf, git reset --hard,
...
# Command Guard: When You Get Blocked
...
```
1. Run `command-guard explain "cmd"` → Understand why (see trace)
...
4. Human approves? → THEY run: command-guard allow-once CODE
```
...
```bash
command-guard doctor              # Health check — hook registered?
command-guard explain "cmd"       # WHY is it blocked? (with trace)
command-guard test "cmd"          # Would this be blocked? (dry-run)
command-guard allow-once CODE     # Human approves (THEY run this)
command-guard packs               # List available rule packs
command-guard scan --staged       # Pre-commit: scan for issues
```
...
**`command-guard explain` example (7-step pipeline):**
```bash
...
```
❌ "Command blocked. Run command-guard allow-once ab12"  → Find alternative first!
...
```toml
# .command-guard.toml — enable rule packs per-project
[packs]
...
- `command-guard_PACKS="containers.docker,kubernetes"` — Enable packs
- `command-guard_DISABLE="kubernetes.helm"` — Disable specific packs
- `command-guard_BYPASS=1` — Escape hatch (human-only)

## Key Facts

- **49+ rule packs** available (database, containers, k8s, cloud, etc.)
- **Sub-millisecond latency** — won't slow your workflow
- **Fail-open on timeout** — if command-guard hangs, command runs (with warning)
...
```bash
# Quick health check
command-guard doctor | head -20

# Test if a command would be blocked
command-guard test "git reset --hard HEAD"
...
| Script | Usage |
|--------|-------|
| `./scripts/validate-command-guard.sh` | Full installation validation |

---

## References

- [COMMANDS.md](references/COMMANDS.md) — Full CLI reference with `command-guard explain`, `command-guard scan`
...
- [PHILOSOPHY.md](references/PHILOSOPHY.md) — Why command-guard works this way
