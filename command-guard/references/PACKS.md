# command-guard Rule Packs

command-guard uses a modular pack system with 49+ rule packs organized by domain.
...
Enable with `command-guard_PACKS` or in `.command-guard.toml`:
...
```bash
export command-guard_PACKS="database.postgresql,kubernetes.kubectl,cloud.aws"
```

### Via Project Config (.command-guard.toml)
...
### Via User Config (~/.config/command-guard/config.toml)
...
export command-guard_DISABLE="kubernetes.helm"
```
...
command-guard packs --verbose database.postgresql

# Check which packs would match a command
command-guard explain "kubectl delete namespace prod"
```
