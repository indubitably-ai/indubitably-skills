...
❌ "Command blocked. Run `command-guard allow-once 12345`."
...
## 3. rm -rf Typo — command-guard Saved You

**Blocked:** `rm -rf /home/user/project/` (meant `./build`)

command-guard caught the typo. Correct the path:
...
✓ "Blocked as safety measure. Since this is a test database, you can approve with `command-guard allow-once 12345`, or I can show contents first."
...
✓ "Procedure requires this. Blocked because it discards local changes. Approve with `command-guard allow-once 12345` if no local work to keep."
...
1. `command-guard allow-once 12345` for this instance
